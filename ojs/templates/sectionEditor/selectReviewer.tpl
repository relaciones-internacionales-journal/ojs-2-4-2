...
<form id="submit" method="post" action="{url op="selectReviewer" path=$articleId}">
	...
	<input type="text" size="10" name="search" class="textField" value="{$search|escape}" />&nbsp;<input type="submit" value="{translate key="common.search"}" class="btn btn-small" />
</form>

<hr/>
<ul class="searchResults_ListAuhors">
{foreach from=$alphaList item=letter}<li><a href="{url op="selectReviewer" path=$articleId searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</li></a> {/foreach}{*<a href="{url op="selectReviewer" path=$articleId}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a>*}
</ul><br/>
<p><a class="action alt_accept" href="{url op="enrollSearch" path=$articleId}">{translate key="sectionEditor.review.enrollReviewer"}</a><br/>
<a class="action crear_general" href="{url op="createReviewer" path=$articleId}">{translate key="sectionEditor.review.createReviewer"}</a>{foreach from=$reviewerDatabaseLinks item="link"}{if !empty($link.title) && !empty($link.url)}&nbsp;|&nbsp;<a href="{$link.url|escape}" target="_new" class="action">{$link.title|escape}</a>{/if}{/foreach}</p>
<hr/>
...
<tr class="heading" valign="bottom">
	...
	<td width="13%" style="text-align:center">{sort_search key="editor.submissions.lastAssigned" sort="latest"}</td>
...
<tr valign="top">
	...
	<td style="text-align:center">{if $reviewerStats.last_notified}{$reviewerStats.last_notified|date_format:$dateFormatShort}{else}&mdash;{/if}</td>
	<td style="text-align:center">{$reviewerStats.incomplete|default:0}</td>
	<td style="text-align:center">
...
{else}
	<tr>
		<td colspan="2" align="left" class="table_results_pagination">{page_info iterator=$reviewers}</td>
		<td colspan="{$numCols-2}" align="right" class="table_results_pagination">{page_links anchor="reviewers" name="reviewers" iterator=$reviewers searchInitial=$searchInitial searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth sort=$sort sortDirection=$sortDirection}</td>
	</tr>
{/if}
</table>
