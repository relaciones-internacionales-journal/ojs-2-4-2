...
<table width="100%" class="info">
	<tr>
		<td width="28%" colspan="2"><a href="{url op="viewMetadata" path=$submission->getId()}" class="action editar_general">{translate key="submission.reviewMetadata"}</a></td>
	...
	<tr>
		...
		<td colspan="5">
			...
			{if $initialCopyeditFile}
				<a href="{url op="downloadFile" path=$submission->getId()|to_array:$initialCopyeditFile->getFileId():$initialCopyeditFile->getRevision()}" class="file descargar_general">{$initialCopyeditFile->getFileName()|escape}</a>&nbsp;&nbsp;{$initialCopyeditFile->getDateModified()|date_format:$dateFormatShort}
			{else}
	...
	<tr>
		...
		<td colspan="5">
			...
			{if $editorAuthorCopyeditFile}
				<a href="{url op="downloadFile" path=$submission->getId()|to_array:$editorAuthorCopyeditFile->getFileId():$editorAuthorCopyeditFile->getRevision()}" class="file descargar_general">{$editorAuthorCopyeditFile->getFileName()|escape}</a>&nbsp;&nbsp;{$editorAuthorCopyeditFile->getDateModified()|date_format:$dateFormatShort}
			{/if}
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td colspan="5">
			...
			{if $finalCopyeditFile}
				<a href="{url op="downloadFile" path=$submission->getId()|to_array:$finalCopyeditFile->getFileId():$finalCopyeditFile->getRevision()}" class="file descargar_general">{$finalCopyeditFile->getFileName()|escape}</a>&nbsp;&nbsp;{$finalCopyeditFile->getDateModified()|date_format:$dateFormatShort}
			{/if}
		</td>
	</tr>
	<tr>
...
<form method="post" action="{url op="uploadCopyeditVersion"}"  enctype="multipart/form-data">
	...
	<p><strong>{translate key="submission.uploadFileTo"}:</strong><br/>
	<label class="checkbox inline" for="copyeditStageInitial"><input type="radio" name="copyeditStage" id="copyeditStageInitial" value="initial" checked="checked" /> {translate key="navigation.stepNumber" step=1}</label>
	<label class="checkbox inline" for="copyeditStageAuthor"{if $canUploadCopyedit == 1} class="disabled"{/if}><input type="radio" name="copyeditStage" id="copyeditStageAuthor" value="author"{if $canUploadCopyedit == 1} disabled="disabled"{else} checked="checked"{/if} /> {translate key="navigation.stepNumber" step=2}</label> 
	<label class="checkbox inline" for="copyeditStageFinal"{if $canUploadCopyedit != 3} class="disabled"{/if}><input type="radio" name="copyeditStage" id="copyeditStageFinal" value="final"{if $canUploadCopyedit != 3} disabled="disabled"{else} checked="checked"{/if} /> {translate key="navigation.stepNumber" step=3}</label><br/><br/>
	<span class="btn btn-file btn-small"><input type="file" name="upload" size="10" class="uploadField"{if !$canUploadCopyedit} disabled="disabled"{/if} />{translate key="plugins.block.navigation.journalContent"}</span>
	<input type="submit" value="{translate key="common.upload"}" class="btn btn-small"{if !$canUploadCopyedit} disabled="disabled"{/if} />
	</p>
</form>
