...
{if !$publicationFeeEnabled || $publicationPayment}
	<form action="{url op="scheduleForPublication" path=$submission->getId()}" method="post">
		<p><strong>{translate key="editor.article.scheduleForPublication"}</strong></p>
				{if $publishedArticle}
					{assign var=issueId value=$publishedArticle->getIssueId()}
				{else}
					{assign var=issueId value=0}
				{/if}
				<select name="issueId" id="issueId" class="selectMenu input-xlarge">
			...
			<br/>
				<input type="submit" value="{translate key="common.record"}" class="btn btn-small" /><br/><br/>
				{if $issueId}
					{if $isEditor}
						<a href="{url op="issueToc" path=$issueId}" class="action buscar_general">{translate key="issue.toc"}</a>
					{else}
						<a href="{url page="issue" op="view" path=$issueId}" class="action buscar_general">{translate key="issue.toc"}</a>
					{/if}
				{/if}

	</form>
	{if $publishedArticle}<hr/>
