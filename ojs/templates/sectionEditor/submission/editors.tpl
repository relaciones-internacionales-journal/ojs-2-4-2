...
<table width="100%" class="listing">
	...
	{/if}
		<tr valign="top">
			...
			{if $isEditor}
				<td><a href="{url page="editor" op="deleteEditAssignment" path=$editAssignment->getEditId()}" class="action delete">{translate key="common.delete"}</a></td>
			{/if}
		</tr>
	{foreachelse}
...
{if $isEditor}
	...
	<a href="{url page="editor" op="assignEditor" path="sectionEditor" articleId=$submission->getId()}" class="action crear_general">{translate key="editor.article.assignSectionEditor"}</a>
	|&nbsp;<a href="{url page="editor" op="assignEditor" path="editor" articleId=$submission->getId()}" class="action crear_general">{translate key="editor.article.assignEditor"}</a>
	{if !$selfAssigned}|&nbsp;<a href="{url page="editor" op="assignEditor" path="editor" editorId=$userId articleId=$submission->getId()}" class="action crear_general">{translate key="common.addSelf"}</a>{/if}
{/if}
</form>
</div>
