...
{else}
	<tr>
		<td colspan="2" align="left" class="table_results_pagination">{page_info iterator=$eventLogEntries}</td>
		<td colspan="2" align="right" class="table_results_pagination">{page_links anchor="eventLogEntries" name="eventLogEntries" iterator=$eventLogEntries}</td>
	</tr>
{/if}
</table>

{if $isEditor}
<a href="{url op="clearSubmissionEventLog" path=$submission->getId()}" class="action delete" onclick="return confirm('{translate|escape:"jsparam" key="submission.event.confirmClearLog"}')">{translate key="submission.history.clearLog"}</a>
{/if}
</div>
