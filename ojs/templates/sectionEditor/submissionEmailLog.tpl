...
<div id="editor_top_menu">
	...
	<li class="active"><a href="{url op="submissionEmailLog" path=$submission->getId()}">{translate key="submission.history.submissionEmailLog"}</a></li>
...
{else}
	<tr>
		<td colspan="3" align="left" class="table_results_pagination">{page_info iterator=$emailLogEntries}</td>
		<td colspan="2" align="right" class="table_results_pagination">{page_links anchor="emailLogEntries" name="emailLogEntries" iterator=$emailLogEntries}</td>
	</tr>
{/if}
</table>

{if $isEditor}
<a class="action delete" href="{url op="clearSubmissionEmailLog" path=$submission->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="submission.email.confirmClearLog"}')">{translate key="submission.history.clearLog"}</a>
{/if}
</div>
