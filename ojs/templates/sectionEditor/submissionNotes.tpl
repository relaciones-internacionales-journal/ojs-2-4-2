...
<div id="editor_top_menu">
	...
	<li class="active"><a href="{url op="submissionNotes" path=$submission->getId()}">{translate key="submission.history.submissionNotes"}</a></li>
</ul>
</div>
...
{elseif $noteViewType == "add"}
	...
	<div class="control-group">
		<label class="control-label">{translate key="common.title"}</label>
		<div class="controls"><input type="text" class="input-xlarge" id="title" name="title" size="50" maxlength="90" class="textField" />
		</div>
	</div>
	<div class="control-group">
		<label class="control-label">{translate key="common.note"}</label>
		<div class="controls"><textarea class="input-xlarge" name="note" id="note" rows="10" cols="50" class="textArea"></textarea></td>
		</div>
	</div>
	<div class="control-group">
		<label class="control-label">{translate key="common.file"}</label>
		<span class="btn btn-file btn-small"><input type="file" name="upload" class="uploadField" />{translate key="plugins.block.navigation.journalContent"}</span>
	</div>
	<br/>
	<input type="submit" class="btn" value="{translate key="submission.notes.createNewNote"}" />
	</form>
{else}
<h3>{translate key="submission.notes"}</h3>
...
{else}
	<tr>
		<td colspan="3" align="left" class="table_results_pagination">{page_info iterator=$submissionNotes}</td>
		<td colspan="3" align="right" class="table_results_pagination">{page_links anchor="submissionNotes" name="submissionNotes" iterator=$submissionNotes}</td>
	</tr>
{/if}
</table>

<a class="action buscar_general" href="javascript:toggleNoteAll()"><div style="display:inline" id="expandNotes" name="expandNotes">{translate key="submission.notes.expandNotes"}</div><div style="display: none" id="collapseNotes" name="collapseNotes">{translate key="submission.notes.collapseNotes"}</div></a> |
<a class="action crear_general" href="{url op="submissionNotes" path=$submission->getId()|to_array:"add"}">{translate key="submission.notes.addNewNote"}</a> |
<a class="action delete" href="{url op="clearAllSubmissionNotes" articleId=$submission->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="submission.notes.confirmDeleteAll"}')">{translate key="submission.notes.clearAllNotes"}</a>
{/if}
</div>
