...
<label class="checkbox" for="installLocale-{$localeKey|escape}"><input type="checkbox" name="installLocale[]" id="installLocale-{$localeKey|escape}" value="{$localeKey|escape}" /> {$localeNames.$localeKey|escape} ({$localeKey|escape})</label>
{if !$localesComplete[$localeKey]}
	<span class="formError">*</span>
	{assign var=incompleteLocaleFound value=1}
{/if}
...
<p> <input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-small" onclick="document.location.href='{url page="admin" escape=false}'" /> <input type="submit" value="{translate key="admin.languages.installLocales"}" class="btn btn-small" /</p>
{/if}
</div>
</form>
