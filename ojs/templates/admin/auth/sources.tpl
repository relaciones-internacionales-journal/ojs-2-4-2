...
<table width="100%" class="listing">
	...
	<tr valign="top">
		...
		<td colspan="2" align="right">
			<input type="submit" value="{translate key="common.save"}" class="btn btn-small" />
		</td>
	</tr>
</table>
...
<form method="post" action="{url op="createAuthSource"}">
	{translate key="common.plugin"}: <select name="plugin" size="1"><option value=""></option>{html_options options=$pluginOptions}</select> <br/><input type="submit" value="{translate key="common.create"}" class="btn btn-small" />
</form>
</div>
</div>
