...
<table class="data_profile" width="100%">
...
{if $currentJournal}
	<tr valign="top">
		...
		<td class="value">
			{if $allowRegReader}
				<label class="checkbox"><input type="checkbox" id="readerRole" name="readerRole" {if $isReader || $readerRole}checked="checked" {/if}/> {fieldLabel name="readerRole" key="user.role.reader"}</label><br/>
			{/if}
			{if $allowRegAuthor}
				<label class="checkbox"><input type="checkbox" id="authorRole" name="authorRole" {if $isAuthor || $authorRole}checked="checked" {/if}/> {fieldLabel name="authorRole" key="user.role.author"}</label><br/>
			{/if}
			{if $allowRegReviewer}
				<label class="checkbox"><input type="checkbox" id="reviewerRole" name="reviewerRole" {if $isReviewer || $reviewerRole}checked="checked" {/if}/> {fieldLabel name="reviewerRole" key="user.role.reviewer"}</label><br/>
			{/if}
		</td>
	</tr>
{/if}
...
<tr valign="top">
	...
	<td class="value">{foreach from=$availableLocales key=localeKey item=localeName}
		<label class="checkbox"><input type="checkbox" name="userLocales[]" id="userLocales-{$localeKey|escape}" value="{$localeKey|escape}"{if in_array($localeKey, $userLocales)} checked="checked"{/if} /> {$localeName|escape}</label><br />
	{/foreach}</td>
</tr>
{/if}
