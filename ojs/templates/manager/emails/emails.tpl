...
{else}
	<tr>
		<td colspan="3" align="left" class="table_results_pagination">{page_info iterator=$emailTemplates}</td>
		<td align="right" colspan="2" class="table_results_pagination">{page_links anchor="emails" name="emails" iterator=$emailTemplates}</td>
	</tr>
{/if}
</table>
...
<a href="{url op="createEmail"}" class="action crear_general">{translate key="manager.emails.createEmail"}</a><br />
<a href="{url op="resetAllEmails"}" onclick="return confirm('{translate|escape:"jsparam" key="manager.emails.confirmResetAll"}')" class="action delete">{translate key="manager.emails.resetAll"}</a>
</div>
