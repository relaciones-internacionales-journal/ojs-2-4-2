	...
	<td class="value">
		...
		{foreach name=responses from=$possibleResponses[$formLocale] key=responseId item=responseItem}
			...
			{/if}
				...
				<tr>
					<td class="label">&nbsp;</td>
					<td class="value"><input type="submit" name="delResponse[{$responseId}]" value="{translate key="common.delete"}" class="btn btn-danger btn-small" /></td>
				</tr>
				<hr/>
		{/foreach}
		...
		{/if}
		<br/>
		<input type="submit" name="addResponse" value="{translate key="manager.reviewFormElements.addResponseItem"}" class="btn btn-small" {if not in_array($elementType, $multipleResponsesElementTypes)}disabled="disabled"{/if}/>
	</td>
</tr>
</table>
<hr/>
<p style="text-align:right;"><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-small" onclick="document.location.href='{url op="reviewFormElements" path=$reviewFormId escape=false}'" /> <input type="submit" value="{translate key="common.save"}" class="btn btn-small" /></p>
</form>
