...
{if $reviewFormElementExists}
	...
	<input type="submit" value="{translate key="common.copy"}" class="btn btn-small"/>&nbsp;<input type="button" value="{translate key="common.selectAll"}" class="btn btn-small" onclick="toggleChecked()" /></p>
	<hr/>
{/if}
</form>
...
<a class="action crear_general" href="{url op="createReviewFormElement" path=$reviewFormId}">{translate key="manager.reviewFormElements.create"}</a>
</div>
