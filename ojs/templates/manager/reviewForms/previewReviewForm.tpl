...
{if $canEdit}
	<div id="editor_top_menu">
			...
			<li class="active"><a href="{url op="previewReviewForm" path=$reviewFormId}">{translate key="manager.reviewForms.preview"}</a></li>
		</ul>
	</div>
{/if}
...
<form id="previewReviewForm" method="post" action="{if !$canEdit}{url op="reviewForms"}{else}{url op="editReviewForm" path=$reviewFormId}{/if}">
	<p style="text-align:right;"><input type="submit" value="{translate key="common.close"}" class="btn btn-small" /></p>
</form>
