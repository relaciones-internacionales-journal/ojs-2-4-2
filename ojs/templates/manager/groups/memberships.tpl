...
<div id="editor_top_menu">
	...
	<li class="active"><a href="{url op="groupMembership" path=$group->getId()}">{translate key="manager.groups.membership}</a></li>
</ul>
</div>
<br/>
...
{else}
	...
	<tr>
		<td align="left" class="table_results_pagination">{page_info iterator=$memberships}</td>
		<td align="right" class="table_results_pagination">{page_links anchor="membership" name="memberships" iterator=$memberships}</td>
	</tr>
{/if}
</table>

<a href="{url op="addMembership" path=$group->getId()}" class="action crear_general">{translate key="manager.groups.membership.addMember"}</a>
</div>
