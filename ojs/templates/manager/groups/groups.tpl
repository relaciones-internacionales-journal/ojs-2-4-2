...
<form action="{url op="setBoardEnabled"}" method="post">
	...
	<label class="checkbox"  for="boardEnabledOff"><input type="radio" id="boardEnabledOff" {if !$boardEnabled}checked="checked" {/if}name="boardEnabled" value="0"/>&nbsp;{translate key="manager.groups.disableBoard"}</label><br/>
	<label class="checkbox"  for="boardEnabledOn"><input type="radio" id="boardEnabledOn" {if $boardEnabled}checked="checked" {/if}name="boardEnabled" value="1"/>&nbsp;{translate key="manager.groups.enableBoard"}</label><br/>
...
{iterate from=groups item=group}
	    ...
	    <tr valign="top" id=editorialteam-{$group->getId()} class="data">
			<td class="drag" width="5%" style="border:none;">&nbsp;</td>
			<td class="drag">
...
{else}
	<tr>
		<td colspan="2" align="left" class="table_results_pagination">{page_info iterator=$groups}</td>
		<td colspan="1" align="right" class="table_results_pagination">{page_links anchor="groups" name="groups" iterator=$groups}</td>
	</tr>
{/if}
</table>

<a href="{url op="createGroup"}" class="action crear_general">{translate key="manager.groups.create"}</a>
</div>
