...
<div id="manager_securitySettings">
<h3>4.1 {translate key="manager.setup.securitySettings"}</h3>
<div id="onlineAccessManagement">
<h4><strong>{translate key="manager.setup.onlineAccessManagement"}</strong></h4>
...
<table width="100%" class="data_alt">
	<tr valign="top">
		...
		<td width="95%" class="value">
			...
			<h4><strong>{translate key="manager.setup.openAccessPolicy"}</strong></h4>
			<p><span class="instruct">{translate key="manager.setup.openAccessPolicyDescription"}</span></p>
			<p><textarea name="openAccessPolicy[{$formLocale|escape}]" id="openAccessPolicy" rows="12" cols="60" class="textArea input-xxlarge"{if $publishingMode != $smarty.const.PUBLISHING_MODE_OPEN} disabled="disabled"{/if}>{$openAccessPolicy[$formLocale]|escape}</textarea></p>
		</td>
	</tr>
	<tr>
...
<div id="siteAccess">
<h4><strong>{translate key="manager.setup.siteAccess"}</strong></h4>
...
<h4><strong>{translate key="manager.setup.userRegistration"}</strong></h4>
...
<div id="manager_publicationScheduling">
<h3>4.2 {translate key="manager.setup.publicationScheduling"}</h3>
<div id="publicationSchedule">
<h4><strong>{translate key="manager.setup.publicationSchedule"}</strong></h4>

<p>{translate key="manager.setup.publicationScheduleDescription"}</p>

<p><textarea name="pubFreqPolicy[{$formLocale|escape}]" id="pubFreqPolicy" rows="12" cols="60" class="textArea input-xxlarge">{$pubFreqPolicy[$formLocale]|escape}</textarea></p>
</div>
<div id="publicationFormat">
<h4><strong>{translate key="manager.setup.publicationFormat"}</strong></h4>
...
<h4><strong>{translate key="manager.setup.frequencyOfPublication"}</strong></h4>
...
<table width="100%" class="data_alt">
	<tr valign="top">
		...
		<td width="80%" class="value"><input type="text" name="initialNumber" id="initialNumber" value="{$initialNumber|escape}" size="5" maxlength="8" class="textField" /></td>
	</tr>
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="initialVolume" key="issue.volume"}</td>
		<td width="80%" class="value"><input type="text" name="initialVolume" id="initialVolume" value="{$initialVolume|escape}" size="5" maxlength="8" class="textField" /></td>
	</tr>
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="initialYear" key="issue.year"}</td>
		<td width="80%" class="value"><input type="text" name="initialYear" id="initialYear" value="{$initialYear|escape}" size="5" maxlength="8" class="textField" /></td>
	</tr>
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="issuePerVolume" key="manager.setup.issuePerVolume"}</td>
		<td width="80%" class="value"><input type="text" name="issuePerVolume" id="issuePerVolume" value="{if $issuePerVolume}{$issuePerVolume|escape}{/if}" size="5" maxlength="8" class="textField" /></td>
	</tr>
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="volumePerYear" key="manager.setup.volumePerYear"}</td>
		<td width="80%" class="value"><input type="text" name="volumePerYear" id="volumePerYear" value="{if $volumePerYear}{$volumePerYear|escape}{/if}" size="5" maxlength="8" class="textField" /></td>
	</tr>
</table>
...
<div id="manager_publicIdentifier">
<h3>4.3 {translate key="manager.setup.publicIdentifier"}</h3>
<div id="uniqueIdentifier">
<h4><strong>{translate key="manager.setup.uniqueIdentifier"}</strong></h4>
...
<h4><strong>{translate key="manager.setup.pageNumberIdentifier"}</strong></h4>
...
<div id="manager_announcementsSection">
...
<p>
	<label class="checkbox" for="enableAnnouncements"><input type="checkbox" name="enableAnnouncements" id="enableAnnouncements" value="1" {if $enableAnnouncements} checked="checked"{/if} />&nbsp;
	{translate key="manager.setup.enableAnnouncements"}</label>
</p>

<p>
	<label class="checkbox" for="enableAnnouncementsHomepage"><input type="checkbox" name="enableAnnouncementsHomepage" id="enableAnnouncementsHomepage" value="1" onclick="toggleEnableAnnouncementsHomepage(this.form)"{if $enableAnnouncementsHomepage} checked="checked"{/if} />&nbsp;
	{translate key="manager.setup.enableAnnouncementsHomepage1"}{translate key="manager.setup.enableAnnouncementsHomepage2"}</label>
...
<h4><strong>{translate key="manager.setup.announcementsIntroduction"}</strong></h4>

<p>{translate key="manager.setup.announcementsIntroductionDescription"}</p>

<p><textarea name="announcementsIntroduction[{$formLocale|escape}]" id="announcementsIntroduction" rows="12" cols="60" class="textArea input-xxlarge">{$announcementsIntroduction[$formLocale]|escape}</textarea></p>
</div><!-- announcementsIntroductionSection -->
</div><!-- announcementsSection -->

<div class="separator"></div>

<div id="manager_copyediting">
...
<h4><strong>{translate key="manager.setup.copyeditInstructions"}</strong></h4>

<p>{translate key="manager.setup.copyeditInstructionsDescription"}</p>

<p>
	<textarea name="copyeditInstructions[{$formLocale|escape}]" id="copyeditInstructions" rows="12" cols="60" class="textArea input-xxlarge">{$copyeditInstructions[$formLocale]|escape}</textarea>
</p>
</div><!-- copyeditInstructionsSection -->

<div class="separator"></div>

<div id="manager_layoutAndGalleys">
<h3>4.6 {translate key="manager.setup.layoutAndGalleys"}</h3>

<p>{translate key="manager.setup.selectOne"}:</p>

<p><label class="checkbox" for="useLayoutEditors-1"><input type="radio" name="useLayoutEditors" id="useLayoutEditors-1" value="1"{if $useLayoutEditors} checked="checked"{/if} />
		{translate key="manager.setup.useLayoutEditors"}</label></p>
<p><label class="checkbox" for="useLayoutEditors-0"><input type="radio" name="useLayoutEditors" id="useLayoutEditors-0" value="0"{if !$useLayoutEditors} checked="checked"{/if} />
		{translate key="manager.setup.noUseLayoutEditors"}</label></p>

<div id="layoutInstructionsSection">
<h4><strong>{translate key="manager.setup.layoutInstructions"}</strong></h4>

<p>{translate key="manager.setup.layoutInstructionsDescription"}</p>

<p>
	<textarea name="layoutInstructions[{$formLocale|escape}]" id="layoutInstructions" rows="12" cols="60" class="textArea input-xxlarge">{$layoutInstructions[$formLocale]|escape}</textarea>
</p>
</div><!-- layoutInstructionsSection -->

<div id="layoutTemplates">
<h4><strong>{translate key="manager.setup.layoutTemplates"}</strong></h4>
...
{foreach name=templates from=$templates key=templateId item=template}
	<tr valign="top">
		...
		<td width="30%"><input type="submit" name="delTemplate[{$templateId|escape}]" value="{translate key="common.delete"}" class="btn btn-danger btn-small" /></td>
{/foreach}
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="template-title" key="manager.setup.layoutTemplates.title"}</td>
		<td width="80%" colspan="2" class="value"><input type="text" name="template-title" id="template-title" size="40" maxlength="90" class="textField input-xlarge" /></td>
	</tr>
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="template-file" key="manager.setup.layoutTemplates.file"}</td>
		<td width="80%" colspan="2" class="value"><input type="file" name="template-file" id="template-file" class="uploadField btn btn-small" /><input type="submit" name="addTemplate" value="{translate key="common.upload"}" class="btn btn-small" /></td>
	</tr>
</table>
</div><!-- layoutTemplates -->

<div id="referenceLinking">
<h4><strong>{translate key="manager.setup.referenceLinking"}</strong></h4>
...
<h4><strong>{translate key="manager.setup.refLinkInstructions.description"}</strong></h4>
<textarea name="refLinkInstructions[{$formLocale|escape}]" id="refLinkInstructions" rows="12" cols="60" class="textArea input-xxlarge">{$refLinkInstructions[$formLocale]|escape}</textarea>
</div><!-- refLinkInstructionsSection -->
</div>
<div class="separator"></div>

<div id="manager_proofreading">
...
<h4><strong>{translate key="manager.setup.proofingInstructions"}</strong></h4>

<p>{translate key="manager.setup.proofingInstructionsDescription"}</p>

<p>
	<textarea name="proofInstructions[{$formLocale|escape}]" id="proofInstructions" rows="12" cols="60" class="textArea input-xxlarge">{$proofInstructions[$formLocale]|escape}</textarea>
</p>
</div>
</div>
