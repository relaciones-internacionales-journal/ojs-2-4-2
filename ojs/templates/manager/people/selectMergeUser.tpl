...
{/strip}
<div id="manager_selectMergeUsers">
...
{else}
	<tr>
		<td colspan="{math equation="floor(numCols / 2)" numCols=$numCols}" align="left" class="table_results_pagination">{page_info iterator=$users}</td>
		<td colspan="{math equation="ceil(numCols / 2)" numCols=$numCols}" align="right" class="table_results_pagination">{page_links anchor="users" name="users" iterator=$users searchInitial=$searchInitial searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth roleSymbolic=$roleSymbolic oldUserIds=$oldUserIds sort=$sort sortDirection=$sortDirection}</td>
	</tr>
{/if}
</table>
{if empty($oldUserIds)}
	<input type="submit" class="btn btn-small" value="{translate key="manager.people.mergeUsers"}" />
	</form>
{/if}
</div>
</div>
