...
<div id="manager_userprofile">
<h3>{$user->getFullName()|escape}</h3>
<div id="profile">
<h4>{translate key="user.profile"}</h4>

<p><a href="{url op="editUser" path=$user->getId()}" class="action editar_general">{translate key="manager.people.editProfile"}</a></p>

<table width="100%" class="data_alt">
	<tr valign="top">
		<td width="20%" class="label">{translate key="user.username"}</td>
		<td width="80%" class="value"><strong>{$user->getUsername()|escape}</strong></td>
	</tr>
	<tr valign="top">
...
{section name=role loop=$userRoles}
	...
	<li>
		...
		{if $userRoles[role]->getRoleId() != $smarty.const.ROLE_ID_SITE_ADMIN}
			<a href="{url op="unEnroll" path=$userRoles[role]->getRoleId() userId=$user->getId() journalId=$userRoles[role]->getJournalId()}" onclick="return confirm('{translate|escape:"jsparam" key="manager.people.confirmUnenroll"}')" class="action delete">{translate key="manager.people.unenroll"}</a>
		{/if}
	</li>
{/section}
{if $notFirstRole}
