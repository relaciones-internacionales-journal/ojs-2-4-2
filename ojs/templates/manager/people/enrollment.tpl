...
<form method="post" action="{url path=$roleSymbolic}">
	...
	<input type="text" size="10" name="search" class="textField" value="{$search|escape}" /><br/><input type="submit" value="{translate key="common.search"}" class="btn btn-small" />
</form>

<hr/>
<ul class="searchResults_ListAuhors">
{foreach from=$alphaList item=letter}<li><a href="{url path=$roleSymbolic searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</a></li> {/foreach}{*<a href="{url path=$roleSymbolic}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a>*}
</ul>
<hr/>
...
{else}
	<tr>
		<td colspan="4" align="left" class="table_results_pagination">{page_info iterator=$users}</td>
		<td align="right" class="table_results_pagination">{page_links anchor="users" name="users" iterator=$users searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth roleSymbolic=$roleSymbolic searchInitial=$searchInitial sort=$sort sortDirection=$sortDirection}</td>
	</tr>
{/if}
</table>
...
{if $userExists}
	...
	<p><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-small" onclick="document.location.href='{url page="manager" escape=false}'" />&nbsp;<input type="button" value="{translate key="common.selectAll"}" class="btn btn-small" onclick="toggleChecked()" />&nbsp;<input type="submit" value="{translate key="email.compose"}" class="btn btn-small"/></p>
{/if}
</div>
</form>

<a href="{url op="enrollSearch" path=$roleId}" class="action crear_general">{translate key="manager.people.enrollExistingUser"}</a> |
...
<a href="{if $roleId}{url op="createUser" roleId=$roleId source=$enrollmentUrl}{else}{url op="createUser" source=$enrollmentUrl}{/if}" class="action crear_general">{translate key="manager.people.createUser"}</a> | <a href="{url op="enrollSyncSelect" path=$rolePath}" class="action reload">{translate key="manager.people.enrollSync"}</a>
</div>
