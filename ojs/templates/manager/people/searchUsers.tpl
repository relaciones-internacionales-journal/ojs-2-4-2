...
{if not $omitSearch}
	...
	<input type="hidden" name="roleId" value="{$roleId|escape}"/>
		...
		<input type="text" size="15" name="search" class="textField" value="{$search|escape}" />&nbsp;<input type="submit" value="{translate key="common.search"}" class="btn btn-small" />
	</form>

	<hr/>
	<ul class="searchResults_ListAuhors">
	{foreach from=$alphaList item=letter}<li><a href="{url op="enrollSearch" searchInitial=$letter roleId=$roleId}">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</a></li> {/foreach}{*<a href="{url op="enrollSearch" roleId=$roleId}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a>*}
	</ul>
	<hr/>
{/if}
