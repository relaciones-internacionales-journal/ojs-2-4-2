		...
		{foreach from=$plugins item=plugin}
			...
			{elseif $managementVerbs}
				{foreach from=$managementVerbs item=verb}
					<a class="action off" href="{url op="plugin" path=$category|to_array:$plugin->getName():$verb[0]}">{$verb[1]|escape}</a>&nbsp;
				{/foreach}
			{/if}
			{assign var=pluginInstallName value=$plugin->getPluginPath()|basename}
			{if $plugin->getCurrentVersion()}
				<a class="action crear_general" href="{url op="managePlugins" path="upgrade"|to_array:$category:$pluginInstallName}">{translate key="manager.plugins.upgrade"}</a>&nbsp;
				<a class="action delete" href="{url op="managePlugins" path="delete"|to_array:$category:$pluginInstallName}">{translate key="manager.plugins.delete"}</a>&nbsp;
			{/if}
			</p></li>
			{/if}
		{/foreach}
