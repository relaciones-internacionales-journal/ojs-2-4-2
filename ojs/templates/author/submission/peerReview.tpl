...
<h4>{translate key="submission.round" round=$round}</h4>
	...
	<p><strong>{translate key="submission.reviewVersion"}: </strong>
		...
		{if $reviewFile}
			<a href="{url op="downloadFile" path=$submission->getId()|to_array:$reviewFile->getFileId():$reviewFile->getRevision()}" class="file descargar_general">{$reviewFile->getFileName()|escape}</a>&nbsp;&nbsp;{$reviewFile->getDateModified()|date_format:$dateFormatShort}
		{else}
			{translate key="common.none"}
		{/if}
	</p>
	<p><strong>{translate key="submission.initiated"}: </strong>
	...
	<p><strong>{translate key="submission.lastModified"}: </strong>
	...
	<p><strong>{translate key="common.uploadedFile"}: </strong>
		{foreach from=$viewableFiles item=reviewerFiles key=reviewer}
			{foreach from=$reviewerFiles item=viewableFilesForReviewer key=reviewId}
				...
				{foreach from=$viewableFilesForReviewer item=viewableFile}
					...
					<a href="{url op="downloadFile" path=$submission->getId()|to_array:$viewableFile->getFileId():$viewableFile->getRevision()}" class="file descargar_general">{$viewableFile->getFileName()|escape}</a>&nbsp;&nbsp;{$viewableFile->getDateModified()|date_format:$dateFormatShort}<br />
				{/foreach}
			{/foreach}
		{foreachelse}
			{translate key="common.none"}
	...
	{if !$smarty.section.round.last}
		...
		<p><strong>{translate key="submission.authorVersion"}: </strong>
			{foreach from=$authorFiles item=authorFile key=key}
				<a href="{url op="downloadFile" path=$submission->getId()|to_array:$authorFile->getFileId():$authorFile->getRevision()}" class="file descargar_general">{$authorFile->getFileName()|escape}</a>&nbsp;&nbsp;{$authorFile->getDateModified()|date_format:$dateFormatShort}<br />
			{foreachelse}
				{translate key="common.none"}
			{/foreach}
		</p>
