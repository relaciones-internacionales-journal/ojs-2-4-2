...
<a href="{url op="viewMetadata" path=$proofSignoff->getAssocId()}" class="action editar_general" target="_new">{translate key="submission.reviewMetadata"}</a>
...
&nbsp;&nbsp;
<a href="javascript:openHelp('{url op="instructions" path="proof"}')" class="action info_general">{translate key="submission.proofread.instructions"}</a>
{/if}
</div>
