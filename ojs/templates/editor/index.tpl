...
{/strip}
<div id="editor_home_articleSubmissions">
<h3>{translate key="article.submissions"}</h3>

<ul class="plain">
	<li class="unassigned"><a href="{url op="submissions" path="submissionsUnassigned"}">{translate key="common.queue.short.submissionsUnassigned"}</a>&nbsp;({if $submissionsCount[0]}{$submissionsCount[0]}{else}0{/if})</li>
	<li class="search"><a href="{url op="submissions" path="submissionsInReview"}">{translate key="common.queue.short.submissionsInReview"}</a>&nbsp;({if $submissionsCount[1]}{$submissionsCount[1]}{else}0{/if})</li>
	<li class="edit"><a href="{url op="submissions" path="submissionsInEditing"}">{translate key="common.queue.short.submissionsInEditing"}</a>&nbsp;({if $submissionsCount[2]}{$submissionsCount[2]}{else}0{/if})</li>
	<li class="file"><a href="{url op="submissions" path="submissionsArchives"}">{translate key="common.queue.short.submissionsArchives"}</a></li>
	{call_hook name="Templates::Editor::Index::Submissions"}
</ul>
</div>

<div id="editor_home_issues">
<h3>{translate key="editor.navigation.issues"}</h3>

<ul class="plain">
	<li class="add"><a href="{url op="createIssue"}">{translate key="editor.navigation.createIssue"}</a></li>
	<li class="notify"><a href="{url op="notifyUsers"}">{translate key="editor.notifyUsers"}</a></li>
	<li class="newissue"><a href="{url op="futureIssues"}">{translate key="editor.navigation.futureIssues"}</a></li>
	<li class="pastissue"><a href="{url op="backIssues"}">{translate key="editor.navigation.issueArchive"}</a></li>
...
<div id="editor_home_search">
	<h3>{translate key="search.searchFor"}</h3>
	<form method="post" id="submit" action="{url path="search"}">
		...
		<br/><span style="text-transform:uppercase;font-weight:bold;">{translate key="common.between"}</span><br/>
		{html_select_date prefix="dateFrom" time=$dateFrom all_extra="class=\"selectMenu\"" year_empty="" month_empty="" day_empty="" start_year="-5" end_year="+1"}
		<br/><span style="text-transform:uppercase;font-weight:bold;">{translate key="common.and"}</span><br/>
		{html_select_date prefix="dateTo" time=$dateTo all_extra="class=\"selectMenu\"" year_empty="" month_empty="" day_empty="" start_year="-5" end_year="+1"}
		<input type="hidden" name="dateToHour" value="23" />
		<input type="hidden" name="dateToMinute" value="59" />
		<input type="hidden" name="dateToSecond" value="59" />
		<br/>
		<input type="submit" value="{translate key="common.search"}" class="btn btn-small" style="margin-top:20px;"/>
	</form>
&nbsp;
</div>
