...
<table class="data_alt" width="100%">
		...
		{foreach from=$article->getAuthors() item=author key=key}
			<tr valign="top">
				...
				<td width="80%" class="value">
					<input type="text" name="searchTerm{$key+1}" value="{$author->getFullName()|escape}" length="40" class="textField" />
				</td>
			</tr>
		{/foreach}
	{elseif $context->getGeoTerms()}
		<tr valign="top">
			<td width="20%" class="label">{translate key="rt.context.termToDefine"}</td>
			<td width="80%" class="value"><input type="text" name="searchTerm" value="{$coverageGeo|escape}" length="40" class="textField" />
		</tr>
	{else}
		<tr valign="top">
			...
			<td width="80%" class="value">
				{foreach from=$keywords item=keyword name=keywords key=key}
					<input type="text" name="searchTerm{$key+1}" value="{$keyword|trim|escape}" length="40" class="textField" />
	...
	{foreach from=$searchValues key=paramKey item=value}
		<tr valign="top">
			...
			<td width="80%" class="value">
					<input type="text" name="{$paramKey|escape}" value="{$value|escape}" length="40" class="textField" />
			</td>
	{/foreach}
	</form>
</table>
...
<table class="listing" width="100%">
	...
	<tr valign="top">
		...
		<td width="2%">{$key+1}.</td>
		<td width="88%">{$search->getTitle()|escape} <a target="_new" href="{$search->getUrl()|escape}" class="action link_general">{translate key="rt.resourceLink"}</a></td>
	</tr>
...
<a href="http://pkp.sfu.ca/ojs/suggestRt" target="_new" class="action crear_general">{translate key="rt.search.suggest"}</a><br/>
</div>
