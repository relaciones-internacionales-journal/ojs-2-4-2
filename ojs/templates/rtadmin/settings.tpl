...
<div id="enableRT">
<label class="checkbox" for="enabled"><input type="checkbox" {if $enabled}checked="checked" {/if}name="enabled" value="1" id="enabled"/>{translate key="rt.admin.settings.enableReadingTools"}</label></div>
...
<table width="100%" class="data_alt">
	...
	<tr valign="top">
		<td class="label">&nbsp;</td>
		<td>
		<label class="checkbox" for="enableCommentsMode-authenticated"><input type="radio" name="enableCommentsMode" id="enableCommentsMode-authenticated" value="{$commentsOptions.COMMENTS_AUTHENTICATED|escape}"{if $enableComments==$commentsOptions.COMMENTS_AUTHENTICATED} checked="checked"{/if} />&nbsp;{translate key="rt.admin.settings.addComment.authenticated"}</label><br />
		<label class="checkbox" for="enableCommentsMode-anonymous"><input type="radio" name="enableCommentsMode" id="enableCommentsMode-anonymous" value="{$commentsOptions.COMMENTS_ANONYMOUS|escape}"{if $enableComments==$commentsOptions.COMMENTS_ANONYMOUS} checked="checked"{/if} />&nbsp;{translate key="rt.admin.settings.addComment.anonymous"}</label><br />
		<label class="checkbox" for="enableCommentsMode-unauthenticated"><input type="radio" name="enableCommentsMode" id="enableCommentsMode-unauthenticated" value="{$commentsOptions.COMMENTS_UNAUTHENTICATED|escape}"{if $enableComments==$commentsOptions.COMMENTS_UNAUTHENTICATED} checked="checked"{/if} />&nbsp;{translate key="rt.admin.settings.addComment.unauthenticated"}</label><br />
		</td>
	</tr>
</table>
</div>
...
<p style="text-align:right"><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-small" onclick="document.location.href='{url page="rtadmin" escape=false}'" /> <input type="submit" value="{translate key="common.save"}" class="btn btn-small" /></p>
