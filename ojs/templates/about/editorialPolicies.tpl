...
<ul class="plain">
	...
	{*{if $currentJournal->getLocalizedSetting('reviewPolicy') != ''}<li><a href="{$baseUrl}/about/editorialPolicies.html#peerReviewProcess">{translate key="about.peerReviewProcess"}</a></li>{/if} *}
	...
	{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION && $currentJournal->getSetting('enableAuthorSelfArchive')}<li><a href="{$baseUrl}/about/editorialPolicies.html#authorSelfArchivePolicy">{translate key="about.authorSelfArchive"}</a></li>{/if}
	{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION && $currentJournal->getSetting('enableDelayedOpenAccess')}<li><a href="{$baseUrl}/about/editorialPolicies.html#delayedOpenAccessPolicy">{translate key="about.delayedOpenAccess"}</a></li>{/if}
	{if $currentJournal->getSetting('enableLockss') && $currentJournal->getLocalizedSetting('lockssLicense') != ''}<li><a href="{$baseUrl}/about/editorialPolicies.html#archiving">{translate key="about.archiving"}</a></li>{/if}
	{foreach key=key from=$currentJournal->getLocalizedSetting('customAboutItems') item=customAboutItem}
		{if !empty($customAboutItem.title)}
			<li><a href="{$baseUrl}/about/editorialPolicies.html#custom-{$key}">{$customAboutItem.title|escape}</a></li>
		{/if}
	{/foreach}
</ul>

{if $currentJournal->getLocalizedSetting('focusScopeDesc') != ''}
<div id="focusAndScope"><div class="arrow_down_box"><h3>{translate key="about.focusAndScope"}</h3></div>
...
<div class="separator">&nbsp;</div>
 *}
{if $currentJournal->getLocalizedSetting('reviewPolicy') != ''}<div id="peerReviewProcess" style="display:none;"><div class="arrow_down_box"><h3>{translate key="about.peerReviewProcess"}</h3></div>
...
<div id="publicationFrequency"><div class="arrow_down_box"><h3>{translate key="about.publicationFrequency"}</h3></div>
...
<div id="openAccessPolicy"><div class="arrow_down_box"><h3>{translate key="about.openAccessPolicy"}</h3></div>
...
<div id="authorSelfArchivePolicy"><div class="arrow_down_box"><h3>{translate key="about.authorSelfArchive"}</h3> </div>
...
<div id="archiving"><div class="arrow_down_box"><h3>{translate key="about.archiving"}</h3></div>
...
{foreach key=key from=$currentJournal->getLocalizedSetting('customAboutItems') item=customAboutItem name=customAboutItems}
	{if !empty($customAboutItem.title)}
		<div id="custom-{$key|escape}"><div class="arrow_down_box"><h3>{$customAboutItem.title|escape}</h3></div>
