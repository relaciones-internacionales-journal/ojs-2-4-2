...
<tr valign="top">
	...
	<td width="80%" class="value"><input type="text" id="label" name="label" value="{$label|escape}" size="40" maxlength="32" class="textField" />
	<span class="help-block">{translate key="submission.layout.galleyLabelInstructions"}</span></td>
</tr>
...
{else}
	<tr valign="top">
		...
		<td class="value"><a class="action descargar_general" href="{url op="downloadFile" path=$articleId|to_array:$galley->getFileId()}">{$galley->getFileName()|escape}</a></td>
	</tr>
	<tr valign="top">
...
<tr valign="top">
	...
	<td class="value">
		<input type="file" name="styleFile" id="styleFile" class="uploadField btn btn-small" />&nbsp;&nbsp;{translate key="form.saveToUpload"}
	</td>
</tr>
</table>
</div>
...
<p style="text-align:right;"><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-small" onclick="document.location.href='{url op="submissionEditing" path=$articleId escape=false}'" /> <input type="submit" value="{translate key="common.save"}" class="btn btn-small" /></p>
</form>
</div>
