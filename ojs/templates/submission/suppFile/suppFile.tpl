...
<table width="100%" class="data_alt">
	...
	<tr valign="top">
		...
		<td class="value">
			...
			<br />
			<span class="help-block">{translate key="author.submit.suppFile.publisherDescription"}</span>
		</td>
	</tr>
	<tr valign="top">
	...
	<tr valign="top">
		...
		<td class="value">
			...
			<br />
			<span class="help-block">{translate key="author.submit.suppFile.dateDescription"}</span>
		</td>
	</tr>
	<tr valign="top">
		...
		<td class="value">
			...
			<br />
			<span class="help-block">{translate key="author.submit.suppFile.sourceDescription"}</span>
		</td>
	</tr>
	<tr valign="top">
		...
		<td class="value">
			...
			<br />
			<span class="help-block">{translate key="author.submit.languageInstructions"}</span>
		</td>
	</tr>
</table>
</div>
...
<table id="suppFile" class="data_alt">
{if $suppFile && $suppFile->getRemoteURL()}
	<tr valign="top">
		...
		<td width="80%" class="value"><input type="text" name="remoteURL" id="remoteURL" value="{$suppFile->getRemoteURL()|escape}" size="40" maxlength="255" class="btn btn-file btn-small" /></td>
	</tr>
</table>
{else}
	...
	<table width="100%"  class="data_alt">
		<tr valign="top">
			...
			<td width="95%" class="value"><label for="showReviewers" class="checkbox"><input type="checkbox" name="showReviewers" id="showReviewers" value="1"{if $showReviewers==1} checked="checked"{/if} />{translate key="author.submit.suppFile.availableToPeers"}</label></td>
		</tr>
	</table>
	{else}
		<tr valign="top">
	...
	<table id="showReviewers" width="100%" class="data_alt">
		<tr valign="top">
			<td class="label">
				{if $suppFile && $suppFile->getFileId()}
		...
		<tr valign="top">
			<td>&nbsp;</td>
			<td class="value">				
				<label for="showReviewers"><input type="checkbox" name="showReviewers" id="showReviewers" value="1"{if $showReviewers==1} checked="checked"{/if} />&nbsp;{translate key="author.submit.suppFile.availableToPeers"}</label>
			</td>
		</tr>
		{/if}
	</table>
...
<p style="float:right;"><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-small" onclick="history.go(-1)" /> <input type="submit" value="{translate key="common.save"}" class="btn btn-small" /></p>

<p style="float:left;">{translate key="common.requiredField"}</span></p>
